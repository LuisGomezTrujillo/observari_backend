...
router = APIRouter(prefix="/api", tags=["ActivityLearner"])

@router.post("/activity-learner", response_model=ActivityLearnerRead)
def create_link(link: ActivityLearnerCreate, session: Session = Depends(get_session)):
    db_link = ActivityLearner(**link.model_dump())
    session.add(db_link)
    session.commit()
    return db_link

@router.get("/activity-learner", response_model=list[ActivityLearnerRead])
def read_links(session: Session = Depends(get_session)):
    return session.exec(select(ActivityLearner)).all()

@router.delete("/activity-learner")
